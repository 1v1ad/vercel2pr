...
router.get('/events', async (req,res)=>{
  try{
    ...
    const events=(r.rows||[]).map(e=>({
      ...
      ip:e.ip, UA:e.ua, ua:e.ua, created_at:e.created_at
    }));
...
router.get('/summary', async (req,res)=>{
  try{
    ...
    res.json({
      ...
      auth7:Number(row.auth7_distinct_users||0),
      unique7:Number(row.unique7_total||0)
    });
  }catch(e){ res.status(500).json({ ok:false, error:String(e?.message||e) }); }
});

// ---------- DAILY (без дублей на пользователя в день) ----------
async function handleDaily(req,res){
  try{
    ...
    const sql=`
      ...
      auth as (
        select (e.created_at at time zone $2)::date d, count(distinct e.user_id) c
        from events e
      ...
      select to_char(d.d,'YYYY-MM-DD') as day, coalesce(a.c,0) as auth, coalesce(u.c,0) as uniq
      from days d left join auth a on a.d=d.d left join uniq u on u.d=d.d
      order by d.d asc
    `;
    const r=await db.query(sql,[days,tz]);
    const rows=(r.rows||[]).map(x=>({ date:x.day, auth:Number(x.auth||0), unique:Number(x.uniq||0) }));
